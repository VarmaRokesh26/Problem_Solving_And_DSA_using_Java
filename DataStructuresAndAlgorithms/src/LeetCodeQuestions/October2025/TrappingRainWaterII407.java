...
public class TrappingRainWaterII407 {
    public int trapRainWater(int[][] heightMap) {
        int[] dRow = { 0, 0, -1, 1 };
        int[] dCol = { -1, 1, 0, 0 };
        int numOfRows = heightMap.length, numOfCols = heightMap[0].length;
        ...
        while (!boundary.isEmpty()) {
            ...
            for (int direction = 0; direction < 4; direction++) {
                int neighborRow = currentCell.row + dRow[direction];
                int neighborCol = currentCell.col + dCol[direction];
                
                if (neighborRow >= 0 && neighborCol >= 0 && neighborRow < numOfRows && neighborCol < numOfCols && !visited[neighborRow][neighborCol]) {
